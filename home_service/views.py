...
def Service_dash(request):
    return render(request,'service_dash.html')
...
def Fuelmandash(request):
    return render(request,'fuelmandash.html')
...
def select_vehicle(request,name):
    brands = Selectbrand.objects.all()
    context = {'brands': brands,'name':name}
    return render(request,'select_vehicle.html',context)

def get_models(request):
    brand_id = request.GET.get('brand_id')
    models = Selectmodel.objects.filter(selectbrand_id=brand_id).values('pk', 'name')
